export default function VotreSalarie2({ data, setData, onNext, onPrev, isLastStep, isCurrentStepComplete }) {
  
  const handleSalarieNameChange = (e) => {
    setData({ ...data, SalarieName: e.target.value });
  };
  
  const clearSalarieName = () => {
    setData({ ...data, SalarieName: '' });
  };
...
<div className="text-base" style={{ color: "#0A2C2D" }}>
  ...
  <div className="flex flex-wrap items-center gap-1">
    ...
    <div className="p-3 rounded-lg outline outline-2 outline-offset-[-1px] outline-cyan-700 inline-flex items-center gap-px relative">
      <input
        type="text"
        ...
        style={{
          '--input-width': `${Math.max(
            data?.SalarieName?.length * 8 || 'régisseur  '.length * 8, // Start with placeholder width
