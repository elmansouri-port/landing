export default function VotreSalarie1({ data, setData, onNext, onPrev, isLastStep, isCurrentStepComplete }) {
  const handlesalarie1Select = (salarie1) => {
    setData({ ...data, salarie1 });
  };

  const salarie1Options = [
    ['un artiste', 'un technicien', 'un artiste cadre', 'un technicien cadre']
  ];
  ...
  return (
    <div className="mt-10 pl-4 max-w-2xl bg-[#FCFCFC]">
      <div className="flex flex-col bg-[#FCFCFC]">
        <div className="w-full max-w-[500px] text-left">
          ...
          <div className="flex flex-wrap items-baseline gap-2 pb-5">
            <div className="text-teal-950 text-4xl font-semibold">La rémunération</div>
            <div className="text-[#99C2E1] text-2xl">/ Votre salarié</div>
          </div>
        </div>
      </div>
      <div className="mb-4 text-base" style={{ color: "#0A2C2D" }}>
        ...
        Mon salarié est{' '}
          <span
            className={`px-2 py-0.5 rounded-md ${data?.salarie1
              ? 'bg-[#E7F1F8] text-[#285E86]'
              : 'bg-slate-100 text-[#99C2E1]'
              }`}
          >
            <span className="font-medium">
              {data?.salarie1 || 'un artiste'}
            </span>
          </span>.
        </p>
      </div>
      <div className="max-w-[500px] flex flex-wrap gap-1.5">
        {salarie1Options.flat().map((salarie1) => (
          data?.salarie1 === salarie1 ? (
            <div
              key={salarie1}
              onClick={() => handlesalarie1Select(salarie1)}
              className="p-3 bg-[#E7F1F8] rounded-md outline outline-3 outline-[#9DC4E2] cursor-pointer"
            >
            ...
            <div
              key={salarie1}
              onClick={() => handlesalarie1Select(salarie1)}
              className="p-3 rounded-md outline outline-2 outline-slate-300 cursor-pointer hover:bg-slate-50"
            >
