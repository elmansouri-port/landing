export default function VotreNbBulletin({
  data,
  setData,
  onNext,
  onPrev,
  ...
  const handleNbBulletinSelect = (NbBulletin) => {
    setData({ ...data, NbBulletin });
  };

  const NbBulletinOptions = [["0 à 5", "6 à 25", "26 à 250", "+ de 250"]];
    ...
    <div className="mt-10 pl-4 max-w-2xl bg-[#FCFCFC]">
      ...
      <div className="mb-4 text-base" style={{ color: "#0A2C2D" }}>
        <p>
          ...
          >
            <span className="font-medium">{data?.NbBulletin || "0 à 5"}</span>
          </span>
          bulletins de paies chaque année.
        </p>
      </div>
      <div className="max-w-[500px] flex flex-wrap gap-2.5">
        {NbBulletinOptions.flat().map((NbBulletin) =>
          data?.NbBulletin === NbBulletin ? (
            <div
              key={NbBulletin}
              onClick={() => handleNbBulletinSelect(NbBulletin)}
